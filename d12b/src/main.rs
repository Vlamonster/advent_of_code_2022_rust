    ...
    for y in 0..height {
        for x in 0..width {
            ...
            match graph.nodes.get(&(x, y.wrapping_sub(1))) {
                None => {}
                Some(node) => {
                    if *node + 1 >= *graph.nodes.get(&(x, y)).unwrap() {
                        graph
                            .edges
                            .get_mut(&(x, y))
                            .unwrap()
...
fn main() {
    ...
    unvisited.insert(graph.end);
    paths.insert(graph.end, Some(vec![]));
    ...
    print!(
        ...
        visited
            .iter()
            .filter(|x| *graph.nodes.get(x).unwrap() == 'a' as usize)
            .map(|x| paths.get(x).unwrap().clone().unwrap().len())
            .min()
            .unwrap()
    );
}
